...
# -----------------------------------------
# Feature Engineering (using From Date)
# -----------------------------------------
# We use the start date of the period to create the time features
selected_date = pd.to_datetime(from_date)

year = selected_date.year
month = selected_date.month
day = selected_date.day
day_of_week = selected_date.dayofweek          # 0 = Monday
...
if st.button("🚀 Predict Demand", use_container_width=True):
    ...
    st.metric(
        label=f"Predicted Daily Demand (based on {from_date})",
        value=f"{prediction:.0f} units"
    )

    # Show period info
    days_in_period = (to_date - from_date).days + 1
    st.info(f"Period: **{from_date}** → **{to_date}**  |  {days_in_period} day(s)")
